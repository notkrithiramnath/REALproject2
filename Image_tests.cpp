...
//larger test
  Pixel another_color = {200, 150, 78};//random color
  Image_init(&img, 20, 30);
  Image_fill(&img, another_color);

  // Check that the pixel at (0, 0) is color
  ASSERT_EQUAL(Image_get_pixel(&img, 0, 0).r, another_color.r);
  ASSERT_EQUAL(Image_get_pixel(&img, 0, 0).g, another_color.g);
  ASSERT_EQUAL(Image_get_pixel(&img, 0, 0).b, another_color.b);

  //check that pixel at the middle is the color
  ASSERT_EQUAL(Image_get_pixel(&img, 5, 10).r, another_color.r);
  ASSERT_EQUAL(Image_get_pixel(&img, 10, 5).g, another_color.g);
  ASSERT_EQUAL(Image_get_pixel(&img, 15, 19).b, another_color.b);

  //check that pixel at the end is the color
  ASSERT_EQUAL(Image_get_pixel(&img, 19, 29).r, another_color.r);
  ASSERT_EQUAL(Image_get_pixel(&img, 19, 29).g, another_color.g);
  ASSERT_EQUAL(Image_get_pixel(&img, 19, 29).b, another_color.b);
